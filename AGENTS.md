...
关键概念首次在正文出现时，不能只作为命令参数、表格条目或排障关键词简略提及。正文必须结合当前主题说明该概念是什么、解决什么问题、边界或不负责什么，以及它与当前对象、命令或状态的关系；若专题细节放在后续章节，当前页仍需保留必要的定义和边界，并提供明确链接。表格、命令速查和排障链路中出现的术语同样适用这一要求：要么在本页解释，要么链接到首次完整解释处。附录可以复用命令，但不得成为概念解释的唯一位置。
