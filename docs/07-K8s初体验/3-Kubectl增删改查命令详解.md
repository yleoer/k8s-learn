...
Kubernetes 中的资源对象通常通过 kubectl 完成增删改查。初学阶段，可以先把操作归纳为四类：创建、查看、修改和删除。
...
kubectl create job hello --image=busybox:1.36 --dry-run=client -o yaml -- echo hello
```

常见流程如下：

```bash
kubectl create deployment nginx --image=nginx:1.25 --dry-run=client -o yaml > nginx-deploy.yaml
