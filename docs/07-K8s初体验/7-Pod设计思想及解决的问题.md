...
Kubernetes 选择 Pod 作为最小调度单元，而不是直接调度容器，是因为真实应用往往并非单个进程孤立运行。Pod 为一组需要紧密协作的容器提供共同的运行边界。
...
理解 Pod 的边界后，再看 Deployment、StatefulSet、DaemonSet，就能明白它们本质上都是在不同场景下管理 Pod。
