...
Kubernetes 中的 Pod 拥有独立 IP，但 Pod IP 不是稳定的服务入口。服务发现关注集群内服务间访问、集群外访问集群内服务，以及集群内访问外部依赖等几个访问边界。本章记录 Service、EndpointSlice、DNS、Headless Service 和代理转发等内容，为后续 Ingress、Gateway API、NetworkPolicy 和微服务访问链路提供基础支撑。
...
Service 更偏向东西向访问和基础服务抽象。Ingress 与 Gateway API 更偏向 HTTP、HTTPS 等南北向流量入口，后续章节单独记录。
