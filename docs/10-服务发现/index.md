...
kubectl label --overwrite po foo status=unhealthy
kubectl label po --all status=unhealthy
kubectl label -f po.json status=unhealthy
...
kubectl get po -l app.kubernetes.io/name=MyApp
```
...
```yaml{6-8,10-13} [run-my-nginx.yaml]
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-nginx
