...
Kubernetes 中的 Pod 拥有独立 IP，但 Pod IP 不是稳定的服务入口。服务发现关注集群内服务间访问、集群外访问集群内服务，以及集群内访问外部依赖等几个访问边界。本章记录 Service、EndpointSlice、DNS、Headless Service、代理转发、Ingress 和入口迁移等内容，为后续 Gateway API 深入、NetworkPolicy 和微服务访问链路提供基础支撑。
...
Service 更偏向东西向访问和基础服务抽象。Ingress 更偏向 HTTP、HTTPS 等南北向流量入口；ingress-nginx 退役后的控制器选型与 Gateway API 迁移在本章记录，Gateway API 的完整能力在后续网络入口内容中继续展开。
