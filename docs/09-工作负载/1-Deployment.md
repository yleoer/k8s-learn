...
Deployment 的 `spec.selector.matchLabels` 必须匹配 Pod 模板中的标签，如果二者不匹配，创建请求会被 API 拒绝。在 `apps/v1` 中，Deployment 创建后 `spec.selector` 不可修改，因此前期应规划好稳定标签。
...
> [!WARNING]
> 不要在 `kubectl set image` 之前执行 `kubectl annotate ... --overwrite`。Deployment 控制器会把 Deployment 上的 `change-cause` 注解同步到当前活跃 ReplicaSet，提前修改会导致旧 revision 的说明被覆盖。
...
`maxSurge` 和 `maxUnavailable` 可以使用整数或百分比，默认值均为 25%，二者不能同时为 0。对核心服务而言，通常建议从 `maxUnavailable: 0` 开始，再根据容量和发布速度进行调整。

与发布节奏相关的还有两个字段：`minReadySeconds` 定义新 Pod 就绪后至少保持多久才被视为可用，默认为 0；`progressDeadlineSeconds` 定义发布多久没有进展就在状态中把 `Progressing` 条件置为 `False`，原因为 `ProgressDeadlineExceeded`，默认为 600 秒，控制器此后仍会继续重试。
