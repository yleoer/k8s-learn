...
```bash
kubectl patch cj hello --type merge -p '{"spec":{"suspend":true}}'
kubectl get cj hello
kubectl patch cj hello --type merge -p '{"spec":{"suspend":false}}'
```
...
kubectl get cj mysql-backup
kubectl create job mysql-backup-manual-001 --from=cj/mysql-backup
