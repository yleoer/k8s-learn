...
个人习惯使用 Vim，先将 `visudo` 的默认编辑器切换为 Vim：
...
**所有节点**，若通过 Tigera Operator 安装 Calico，calico-node 需要连接运行 Typha 实例的节点，放行 Typha 通信端口：
...
禁用 swap 是为了确保 kubelet 能够依据真实内存压力做出准确的资源管理与 Pod 驱逐决策，避免因 swap 介入导致内存不足时的行为不可预期。这并不代表 Kubernetes 不支持 swap：自 v1.34 起节点 swap 支持（NodeSwap）已 GA，可通过 kubelet 的 `failSwapOn: false` 与 `swapBehavior: LimitedSwap` 让 Pod 受控地使用 swap，但默认配置下 kubelet 仍拒绝在启用 swap 的节点上启动，本文遵循默认要求禁用。
