...
如果 `raw.githubusercontent.com` 无法访问，可先将 YAML 下载到本地后再创建资源，无需在集群节点上连接该域名：

```bash
# 在可以访问 GitHub 的机器上执行
curl -fsSL -o calico-crd.yaml https://raw.githubusercontent.com/projectcalico/calico/v3.32.1/manifests/v1_crd_projectcalico_org.yaml
curl -fsSL -o tigera-operator.yaml https://raw.githubusercontent.com/projectcalico/calico/v3.32.1/manifests/tigera-operator.yaml

# 将这两个文件传输到 control-plane 节点后创建资源
kubectl create -f calico-crd.yaml
kubectl create -f tigera-operator.yaml
```

本文通过 Tigera Operator 安装 Calico，而非直接提交 Calico 的 DaemonSet 清单。Operator 模式将 Calico 组件的生命周期（部署、配置、升级、扩缩）统一管理，后续调整网络参数或升级 Calico 版本时只需修改 `Installation` 资源，Operator 自行完成组件更新。
...
kubectl delete -f https://raw.githubusercontent.com/projectcalico/calico/v3.32.1/manifests/tigera-operator.yaml --ignore-not-found
kubectl delete -f https://raw.githubusercontent.com/projectcalico/calico/v3.32.1/manifests/v1_crd_projectcalico_org.yaml --ignore-not-found
```
...
worker 节点加入后，回到 control-plane 节点重新执行[配置 kubectl](#配置-kubectl)末尾的节点与 Pod 查询命令，确认新增节点和系统 Pod 状态。
...
部署官方组件清单。本文固定使用 Metrics Server v0.8.1。
...
```bash
kubectl describe no "$(hostname)" | grep -i taints
```
...
- [网络插件与网络策略](https://kubernetes.io/docs/concepts/cluster-administration/addons/#networking-and-network-policy)
