...
| 文档 | 内容 |
| --- | --- |
| [环境规划与版本选择](./1-1-环境规划与版本选择) | 明确版本、节点、网段、主机名和快照策略 |
| [基础环境准备](./1-2-基础环境准备) | 完成系统更新、基础工具、远程访问、内核网络和 swap 配置 |
| [运行时与组件安装](./1-3-运行时与组件安装) | 安装 containerd、配置镜像加速、安装 kubeadm 相关组件 |
| [集群初始化与网络插件](./1-4-集群初始化与网络插件) | 初始化 control-plane、安装 Calico、加入 worker 节点 |
| [验证集群与常见排障](./1-5-验证集群与常见排障) | 部署测试应用并掌握第一轮排障命令 |
