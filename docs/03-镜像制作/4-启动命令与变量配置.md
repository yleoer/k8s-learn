...
检查命令在容器内部执行，必须使用目标镜像内实际存在的工具。`alpine` 基础镜像通常包含 BusyBox `wget` 而不包含 `curl`，但具体应用镜像可能额外安装工具，例如 `nginx:1.31-alpine` 同时包含 `wget` 和 `curl`。检查路径也应指向应用真实提供的端点：静态站点检查 `/`，后端服务通常暴露专门的 `/healthz`。
...
ARG 也可用于控制基础镜像版本，Dockerfile 片段复用前文[FROM：选择基础镜像](./2-基础镜像与镜像元数据.md#from选择基础镜像)中的示例。
