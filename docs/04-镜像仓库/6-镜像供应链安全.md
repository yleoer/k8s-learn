...
镜像相关的准入方式常见有三类，优先考虑稳定 API 或通用 Webhook 方案：
...
| `ImagePolicyWebhook`                  | API Server 调用外部 HTTPS Webhook，由外部服务决定镜像是否允许 | alpha API，不建议作为新生产方案的首选               |

`ImagePolicyWebhook` 能直接处理镜像准入，但它依赖外部策略服务和 alpha 版本的 `ImageReview` 对象。实际生产中，可优先使用 ValidatingAdmissionPolicy、ValidatingAdmissionWebhook 或 Sigstore Policy Controller 等组件，把签名、来源和仓库范围转化为准入规则。

> [!NOTE]
> `imagepolicy.k8s.io/v1alpha1` API 仍在 alpha 阶段，未进入稳定版本。生产环境如果对 API 稳定性要求较高，可优先采用 ValidatingAdmissionPolicy 或 ValidatingAdmissionWebhook 方案。
...
Harbor 可以保存镜像和部分 OCI 制品，自 v2.11 起还支持在仓库侧使用 Trivy 为制品生成、查看和复制 SBOM，并提供漏洞扫描、复制、权限和审计能力；签名策略、准入控制和构建证明的完整闭环，通常还需要构建系统、签名系统和 Kubernetes 准入组件共同完成。
...
- [Docker 构建证明](https://docs.docker.com/build/metadata/attestations/)
- [Docker SBOM 证明](https://docs.docker.com/build/metadata/attestations/sbom/)
- [Docker SLSA 来源证明](https://docs.docker.com/build/metadata/attestations/slsa-provenance/)
...
- [使用 Cosign 签名容器镜像](https://docs.sigstore.dev/cosign/signing/signing_with_containers/)
- [使用 Cosign 验证容器镜像](https://docs.sigstore.dev/cosign/verifying/verify/)
- [Sigstore 策略控制器](https://docs.sigstore.dev/policy-controller/overview/)
