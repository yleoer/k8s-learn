...
控制面负责决策，工作节点负责执行。每个节点上的 kubelet、容器运行时、kube-proxy 和 CNI 插件共同完成 Pod 运行、状态上报和服务转发。
...
常见运行时包括 containerd 和 CRI-O。本课程使用 containerd。kubelet 不依赖具体实现，只要运行时符合 CRI 规范即可。
...
flowchart LR
  ...
  svc --> proxy["kube-proxy 规则（iptables / IPVS）"]
  proxy --> pod1["Pod A"]
  proxy --> pod2["Pod B"]
  proxy --> pod3["Pod C"]
```

它监听 Service 和 EndpointSlice 的变化，动态更新转发规则。部分现代 CNI（如 Cilium）可替代 kube-proxy 实现 Service 转发。
...
常见选择包括 Calico、Cilium 和 Flannel。本课程使用 Calico。网络插件异常时，Pod 可能创建失败、获取错误 IP，或无法跨节点通信。

## CoreDNS

CoreDNS 让应用通过名称访问服务，而不是硬编码 Pod IP：

```text
redis                    # 同一命名空间
redis.default.svc.cluster.local   # 跨命名空间完整域名
```

Pod IP 随重建变化，Service 名称是稳定的。CoreDNS 将 Service 名称解析为 ClusterIP，应用之间始终通过名称调用。

## Metrics Server

Metrics Server 从 kubelet 采集 CPU、内存等指标，供 `kubectl top` 和 HPA 使用：
...
它用于实时指标采集，不适合作为长期监控系统。长期监控通常需要使用 Prometheus 等方案。
