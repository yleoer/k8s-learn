...
CRI（Container Runtime Interface）是 kubelet 与容器运行时之间的 gRPC API 规范。kubelet 面向 CRI 发起请求，不需要关心底层实现是 containerd、CRI-O 还是其他符合接口规范的运行时。
...
如果 `crictl info` 无法连接运行时，需要检查 `/etc/crictl.yaml` 是否存在且 socket 路径正确：
