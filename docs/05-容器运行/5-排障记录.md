...
| 用途 | 命令 |
| --- | --- |
| 节点运行时版本 | `kubectl describe no <node> \| grep "Container Runtime"` |
| Pod 事件 | `kubectl describe po <pod> -n <namespace>` |
| 集群事件排序 | `kubectl get ev -A --sort-by=.metadata.creationTimestamp` |
...
检查镜像所在 namespace。完整命令复用前文[ctr 能看到镜像但 Pod 仍然 ImagePullBackOff](./2-客户端工具与命名空间.md#ctr-能看到镜像但-pod-仍然-imagepullbackoff)中的对比查询。

镜像必须出现在实际使用的 containerd namespace 中，通常是 `k8s.io`，且 Pod 需要设置合适的 `imagePullPolicy`。离线导入镜像时，多节点集群中的每个候选节点都必须具备同一镜像。

## 生产建议

1. 镜像仓库优先使用 HTTPS 和权威 CA；内部 CA 场景应在所有节点统一分发证书。
2. 不应在生产环境长期使用 `skip_verify` 或 HTTP 仓库。
3. 变更节点运行时配置前，先评估业务影响；必要时通过 `kubectl drain` 迁移 Pod 后再操作。注意：`certs.d` 目录下 `hosts.toml` 的变更动态生效，但 `config.toml` 中 `config_path` 的启用或变更仍需要重启 containerd。
...
- `imagePullSecrets` 解决 Pod 拉取镜像时的身份认证，包括用户名、密码或 Token，相关 Secret 必须与 Pod 位于同一命名空间

二者独立工作。证书配置错误时，即使 Secret 正确也会报 `x509`；Secret 错误时，即使证书配置正确也会报 `unauthorized`。

### containerd namespace 和 Kubernetes namespace 是一回事吗？

不是。Kubernetes namespace 是集群 API 层面的资源隔离，用于隔离 Pod、Service 等对象；containerd namespace 是单节点运行时层面的资源隔离，用于隔离镜像、容器和快照。`kubectl get po -n default` 和 `ctr -n default images ls` 中的 `default` 没有关联。Kubernetes 通常使用 containerd 的 `k8s.io` namespace。
