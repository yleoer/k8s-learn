...
从 Docker Engine v29（2025 年 11 月发布）开始，全新安装默认使用 containerd image store 替代传统的 graph driver（如 overlay2），这也意味着 Docker 的镜像存储和容器运行都统一到了 containerd 上。已有升级安装不受影响，继续使用原有存储驱动。
