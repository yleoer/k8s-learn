...
例如，Deployment 声明某个服务需要 3 个副本后，Deployment 和 ReplicaSet 控制器会持续比较期望副本数与实际 Pod 数，并在副本不足或过多时进行调整。只有通过就绪检查的 Pod 才会计入可用副本；节点故障后的补建也只适用于由工作负载控制器管理的 Pod，并不意味着原 Pod 会被迁移到其他节点。
...
| 自愈恢复 | kubelet 按重启策略重启失败容器；工作负载控制器在副本不足时补建 Pod |
...
```mermaid
flowchart TB
  control["Kubernetes 控制面"] <-->|API watch / 状态上报| kubelet["kubelet"]
  kubelet --> cri["CRI"]
