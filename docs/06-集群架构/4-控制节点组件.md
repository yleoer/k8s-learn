...
Scheduler 和 Controller Manager 多副本部署时通过 Leader 选举确保同一时刻只有一个实例工作。自 Kubernetes v1.21 起，Leader 选举默认使用 Lease 机制（而非 ConfigMap/Endpoints），这一改变提高了选举效率并减少了 etcd 写入压力。
