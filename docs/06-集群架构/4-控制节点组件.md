...
cloud-controller-manager 是可选的控制面组件，只在运行于云平台的集群中部署。它将与云厂商 API 交互的控制器（节点生命周期、云负载均衡器、路由配置）从 kube-controller-manager 中拆分出来，使核心组件不绑定具体云平台。自建集群没有云 API 需要对接，不部署该组件，本文档环境同样不涉及。
...
早期版本的选举锁基于 Endpoints 或 ConfigMap 注解实现，自 v1.20 起默认改用专用的 Lease 资源，旧的锁类型已在后续版本中移除。选举心跳只更新 Lease 对象，不再让监听 Endpoints、ConfigMap 的组件收到无关的 watch 事件。
