...
cloud-controller-manager 是可选的控制面组件。当集群需要调用外部云提供商 API 时，它会承载节点生命周期、云负载均衡器和路由等控制逻辑，使核心组件不绑定某一家云平台。是否部署取决于集群是否配置了这类云集成，而不是是否通过 kubeadm 自建；本文档环境没有此类依赖，因此不涉及该组件。
...
Lease 还用于节点心跳。每个 Node 在 `kube-node-lease` 命名空间中都有同名 Lease，kubelet 会定期更新其续约时间；控制面据此辅助判断节点是否仍在正常通信。

## Deployment 控制面链路

1. `kubectl create deploy` 请求进入 APIServer，完成认证、校验并写入 etcd。
...
- [控制平面与节点通信](https://kubernetes.io/docs/concepts/architecture/control-plane-node-communication/)
