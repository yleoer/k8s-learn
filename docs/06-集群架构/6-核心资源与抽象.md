...
Kubernetes 概念较多，但每一个概念都不是凭空设计的。它们将应用运行中的不同问题拆分为不同资源对象，每个对象负责解决一类具体问题。
...
| 服务发现与入口 | Service、EndpointSlice、Ingress、Gateway API | 为动态 Pod 提供稳定访问入口 |
| 配置 | ConfigMap、Secret、Downward API | 把配置从镜像中拆分出来 |
...
Pod 是 Kubernetes 中最小的可部署计算单元，也是容器运行的原子边界，但在生产实践中很少直接使用裸 Pod。Deployment 管理副本和更新策略；Service、Ingress 和 Gateway API 提供访问入口；ConfigMap、Secret、PVC 注入运行时依赖。
...
| DaemonSet | 匹配节点覆盖（日志采集、监控 Agent、网络组件） |
| Job | 一次性任务（数据迁移、初始化） |
| CronJob | 定时任务（备份、清理、报表） |

Deployment 关注副本数和平滑更新；StatefulSet 关注稳定网络标识和存储持久化；DaemonSet 关注节点覆盖；Job/CronJob 关注任务完成状态。

Eureka 属于特定 Spring Cloud / Netflix 技术栈语境中的注册中心示例。Kubernetes 集群内部的服务发现通常优先依赖 Service 和 DNS；需要分布式协调能力时，ZooKeeper、etcd 等示例更贴近 StatefulSet 的稳定身份和存储特征。
...
Namespace 本身只负责逻辑分组，完整隔离还需要配合 RBAC、ResourceQuota、LimitRange 和 NetworkPolicy，分别控制访问权限、命名空间资源总量、单个 Pod 与容器的资源范围和网络访问边界。
...
| 服务发现 | Service、EndpointSlice |
| 外部流量接入 | Ingress、Gateway API |
| 配置与镜像分离 | ConfigMap、Secret |
| 数据持久化 | PV、PVC、StorageClass |
| 团队隔离 | Namespace、RBAC、Quota |
| 任务执行 | Job、CronJob |

Kubernetes 资源记录不必一次列完所有字段，而应先明确每个资源解决的问题。后续章节会围绕这些资源逐一展开实操。
