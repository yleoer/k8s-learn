...
Pod IP 随重建变化，Service 名称是稳定的。CoreDNS 将普通 Service 名称解析为 ClusterIP；对于 Headless Service，则返回匹配后端的 DNS 记录。`cluster.local` 是常见默认集群域名后缀，可通过 kubelet 或 kubeadm 的集群 DNS 配置调整。应用之间应通过名称调用，而不是硬编码 Pod IP。

## Metrics Server

Metrics Server 从 kubelet 采集短期 CPU、内存资源指标，供 `kubectl top` 和 HPA 使用：
