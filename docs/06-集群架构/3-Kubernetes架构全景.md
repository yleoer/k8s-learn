...
控制平面（Control Plane）是集群的大脑，负责状态管理和决策；其组件通常集中部署在专用的控制节点上，这些节点一般不承载业务 Pod：
...
运行在云平台上的集群还有一个可选组件 cloud-controller-manager，承载与云厂商 API 交互的控制逻辑；自建集群不部署该组件。
...
| kube-proxy | 维护 Service 转发规则（iptables、nftables 或已弃用的 ipvs） |
